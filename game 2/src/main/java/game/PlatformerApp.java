...
FXGL.getPhysicsWorld().addCollisionHandler(new CollisionHandler(EntityType.PLAYER, END_DOOR) {
        @Override
        protected void onCollision(Entity player, Entity Entity end_door;
        end_door) {
            if (hasKey) {
                map_level = 1;
                level_count = 0;
                ...
                getDialogService().showInputBox("Je tijd is " + s + " seconden. \n Voer je naam in: ", answer -> {
                    ...
                    highscoreManager.refresh(); // Add this line to refresh the high scores after a new score has been added.
